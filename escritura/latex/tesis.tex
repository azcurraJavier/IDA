...
Siguiendo con los conceptos mencionados los modelos mentales son representaciones mentales que el programador elabora las cuales mapean a distintas componentes del sistema. Estas representaciones pueden ser explayadas en diagramas conocidos en el ámbito de la ingeniería del software como \textit{Unified Modeling Language} UML, \textit{Entity Relationship} ER entre otros.
...
Hasta aquí solo se ha cumplido una parte del objetivo principal porque solo se hablo de extraer información de los dominios del problema y del programa. Desafortunadamente el mapeo que relaciona la salida del sistema con los componentes que la integran queda a manos del programador y no abundan herramientas automatizadas que simplifiquen esta tarea.
...
Actualmente existen técnicas que conectan visualmente el dominio del problema y el dominio del programa usando la información estática y dinámica que se extrajo del sistema de estudio.

Una de las técnicas conocidas es \textit{Simultaneous Visualization Strategy} SVS, se encarga de mostrar los distintos componentes de un programa en plena ejecución, mediante distintas vistas, usando un inspector de sentencias, de esta manera se obtiene una visualización cuando el sistema se está ejecutando.
La idea es usar un esquema de instrumentación de código descripto en la sección anterior, donde las acciones semánticas le van indicando a un visualizador la traza de invocaciones a funciones durante la ejecución del sistema.
...
Para la construcción de vistas se requieren un conjunto de librerías gráficas. Dichas librerías son programas que simplifican la elaboración de las vistas las cuales poseen distintas características y el programador tiene la tarea de elegir que librería es la mas adecuada para la vista que desea construir.
...
En el capítulo anterior se introdujo en el ámbito de comprensión de programas con la definición de los conceptos mas importantes. Este capitulo se va a centrar en el estado del arte actual sobre técnicas conocidas en el análisis de ids para hacer un aporte al estudio encargado de facilitar la extracción de información estática en los códigos de los sistemas. 

Un Identificador (id) es una secuencia de caracteres alfanuméricos de cualquier longitud que sirve para identificar las entidades del programa (clases, funciones, variables, tipos estructurados, etc.). 
...
Varios autores Feild, Binkley, Lawrie [ref] distinguen 2 formas de nombramiento de los identificadores conocidas como \textit{hardwords} y \textit{softwords}, las primeras destacan la separación de cada palabra que compone el identificador a través de una marca específica. Por ejemplo los ids: fileSystem, file\_system, fileSYSTEM marcan bien la separación de cada palabra con el uso de mayúscula, minúscula o con el guion bajo. Las segundas no poseen ningún tipo de separador por ejemplo: textinput compuesta por text-input.
...
Los identificadores que generalmente son abreviaciones, ocultan información relevante del domino del problema. Una manera de desplegar esta información oculta es intentar convertir las abreviaturas en palabras completas del lenguaje natural. 

Se puede decir que el foco del análisis de los identificadores se basa en la traducción de palabras abreviadas a palabras completas.

Tanto los nombres de los identificadores como las abreviaturas utilizadas dependen mucho de la idiosincrasia del programador y esto representa un problema para traducir su significado.

La técnicas encargadas de realizar la conversión de los ids a palabras completas se dividen en dos etapas: la división de los ids y expansión de los ids [ref].
