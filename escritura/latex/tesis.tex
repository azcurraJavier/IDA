...
\pagebreak 
\textbf{Identifier Restructuring tool\\}
...
La herramienta restructuring tool se encarga de recibir como entrada un código fuente escrito en lenguaje C y luego a través de un proceso de transformación cada nombramiento de los id se expande a palabras completas, retornando de esta manera el mismo código pero con los ids reestructurados.
...
\item \textbf{Identifier Restructurer:} Genera una asociación entre el nombramiento actual del id y un nombramiento estándar mas claro. El primer paso consiste en segmentar el id en las palabras que lo constituyen, después cada palabra debe pertenecer a un diccionario de palabras estándar (estándar léxico) finalmente la secuencia de palabras en los ids deben coincidir con reglas predefinidas por una gramática (estándar sintáctico).
\item \textbf{Identifier Replacer:} Transforma el código original en el nuevo código usando las asociaciones que se construyeron en la etapa anterior. Se usa un scanner léxico para evitar reemplazar posibles nombres de ids contenidos en literales y en comentarios.
\end{itemize}

Los pasos 1 y 3 están totalmente automatizados. Sin embargo no sucede lo mismo con el paso 2 porque se necesita la intervención del programador para lograr una expansión adecuada de nombres. A continuación se detalla el paso 2 que es el mas importante de esta herramienta.

\begin{figure}[h] %[h] para here [b] para bottom [t] para top
\centering
\includegraphics[scale= 0.73]{./ire_2.png}
