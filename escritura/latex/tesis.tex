...
El proceso automático que se lleva a cabo para realizar la traducción de ids consta de dos pasos{LFBEX07}:
...
\pagebreak
\subsection{Algoritmo de Expansión 1}
%noindent elimina sangria
...
\subsection{Algoritmo de Expansión 2}
%noindent elimina sangria
%\noindent \textbf{\\Algoritmo 2}

El algoritmo de expansión de abreviaturas que construyó Emily Hill, Zachary Fry, Haley Boyd\cite{EZH08} ademas de buscar potenciales expansiones al igual que el algoritmo anterior, también se encarga de seleccionar la que mejor se ajusta en caso de que haya mas de una elección y de esta forma siempre devuelve un resultado.

Una mejora destacable con respecto al algoritmo previo es que no se necesita un diccionario con palabras en lenguaje natural. Los diccionarios (en inglés) incluyen demasiadas palabras e implica disponer de un gran almacenamiento. Las listas de abreviaciones comunes y palabras irrelevantes son armadas a partir de distintos softwares y pueden ser ajustadas a mano a la necesidad del usuario.

Para agilizar la lectura se asigna el nombre de `palabras abreviadas' aquellas que necesitan expandirse y `palabras largas' a las normales que no están abreviadas.
...
Se comienza con un alcance mas cercano donde se examina sentencias vecinas a la palabra abreviada. Luego gradualmente el alcance crece para incluir métodos, sus comentarios, y los comentarios de la clase. Si la técnica no encuentra una palabra larga adecuada para la abreviada la búsqueda continua mirando todo el programa y finalmente en JAVA SE 1.5. Los autores asumen que para una palabra abreviada esta asociada a una sola palabra larga dentro de un método. En caso de que esto no se cumpla se puede achicar el alcance de esta asunción a bloques o incluso a sentencias de código.\\


\noindent \textbf{Expansión Mas Frecuente (MFE)\\}

La estrategia MFE mejora la precisión de acertar con la expansión de una abreviatura. La idea consiste en ejecutar ---- sobre el programa entero. Para cada palabra abreviada, se cuenta el numero de veces que esa palabra se le asigna una determinada palabra larga. Se calcula la frecuencia relativa que una palabra abreviada fue expandida con cada palabra larga. La palabra larga con mayor frecuencia relativa se considera la expansión mas frecuente. Al final se agrupan (tambien) las palabras largas con las mismas chances de ser las correctas cuando se crea la lista de MFE.

Sin embargo suele pasar que la expansión mas probable es la incorrecta. Para evitar que suceda se considera a su vez, una palabra larga debe superar mas de la mitad (0.5) en la frecuencia de aparición con respecto a una abreviatura y también que la palabra abreviada haya sido elegida al menos 3 veces en todo el programa.%Aclara mas esto

La técnica MFE tiene 2 niveles, el primero es el nivel de programa y uno mas general a nivel JAVA. El nivel de programa es ideal ya que expande las abreviaturas con palabras propias del dominio del problema. El nivel mas general se arma con la API de Java. En la tabla \ref{t} se muestra algunos casos de frecuencias mas altas de JAVA 5. En caso de que una palabra abreviada no obtenga un candidato de expansión, MFE se puede entrenar sobre una gran cantidad de programas JAVA para encontrarlo. Existe la posibilidad de armar una lista a mano para casos puntuales que ayude a mejorar la precisión de MFE. Otras soluciones propuestas son documentación online relacionada a JAVA o vinculada a la ciencias de computación en general.
...
A continuación se presenta una herramienta que basa su análisis solo en ids nombrados de la forma hardword. La herramienta realiza una reestructuración de nombres de ids. Los nombres se cambian por nombres mas explicativos agregando un verbo que indica la función del id en el código, mas precisamente después de renombrar los ids se visualiza claramente el rol que cumple el id en el programa\cite{EZH08,DCHD06,DLHD06,LFBEX07,BCPT00,BCPT99}.
